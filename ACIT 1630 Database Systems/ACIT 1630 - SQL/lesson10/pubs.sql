...
select pub_id, title_id, price, pubdate, avg(price) over (partition by pub_id)
from titles
...
select type, pub_id, titles.title_id, au_ord,
   Name = substring (au_lname, 1,15), ytd_sales, avg(ytd_sales) over (partition by pub_id, type)
...
@type char(12)
AS
select pub_id, type, title_id, price, count(title_id) over (order by pub_id, type)
from titles
